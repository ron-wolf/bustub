...
template <typename KeyType, typename ValueType, typename KeyComparator>
bool HASH_TABLE_TYPE::GetValueLatchFree(Transaction *transaction, const KeyType &key, std::vector<ValueType> *result) {
  HashTableHeaderPage *header_page = GetHeaderPage();
  size_t num_blocks = header_page->NumBlocks();
  size_t size = header_page->GetSize();
...
bool HASH_TABLE_TYPE::Insert(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  success = buffer_pool_manager_->UnpinPage(block_page_id, true, nullptr);
  assert(success);
  success = buffer_pool_manager_->UnpinPage(header_page_id_, false, nullptr);
  assert(success);
...
bool HASH_TABLE_TYPE::Remove(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  while (block_page->IsOccupied(bucket_ind)) {
    if (block_page->IsReadable(bucket_ind) && comparator_(key, block_page->KeyAt(bucket_ind)) == 0 &&
        value == block_page->ValueAt(bucket_ind)) {
      success = true;
...
template <typename KeyType, typename ValueType, typename KeyComparator>
void HASH_TABLE_TYPE::Resize_Insert(HashTableHeaderPage *header_page, const KeyType &key, const ValueType &value) {
...
template <typename KeyType, typename ValueType, typename KeyComparator>
void HASH_TABLE_TYPE::CreateNewBlockPages(HashTableHeaderPage *header_page, size_t num_blocks) {
  // TODO: create all the necessary blocks, and store page_ids in the header file
  for (size_t i = 0; i < num_blocks; i++) {
    page_id_t new_block_page_id = INVALID_PAGE_ID;
    Page *page = buffer_pool_manager_->NewPage(&new_block_page_id, nullptr);
    assert(page != nullptr);
    GetHeaderPage()->AddBlockPageId(new_block_page_id);
    [[maybe_unused]] bool success = buffer_pool_manager_->UnpinPage(new_block_page_id, true, nullptr);
    assert(success);
  }
}
...
void HASH_TABLE_TYPE::Resize(size_t initial_size) {
  ...
  size_t old_num_blocks = old_header_page->NumBlocks();  // IDK: where would I use this?
  ...
  [[maybe_unused]] bool success;
  success = buffer_pool_manager_->UnpinPage(block_page_id, false, nullptr);
  assert(success);
  DeleteBlockPages(old_header_page);

  success = buffer_pool_manager_->UnpinPage(header_page_id_, true, nullptr);
  assert(success);
  success = buffer_pool_manager_->UnpinPage(old_header_page_id, false, nullptr);
  assert(success);
